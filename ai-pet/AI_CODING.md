...
| 再接続のE2E | localhost では再接続が速すぎてポーリングでは途中の表示を取りこぼす。`MutationObserver` で履歴を見る作りになっている |
...
- いのししは絵が横長なので**描画側で1.3倍**している（`sprites.ts` の `SPECIES_SCALE`）
- 睡眠は `alpha = 0.75` だけ（ポーズのアセットが無い）
- 歩行は上下バウンドで代用（コマ画像が無い）
...
- **巣（nest）タイルは WeakMap 保持なので再起動すると忘れる**（永続化するなら `Actor` に持たせる）
...
まだ `constants.ts` に移せていないもの: `critter.ts` の `WEIGHTS`。
