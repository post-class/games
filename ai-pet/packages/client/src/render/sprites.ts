 ...
 * いのししの表示倍率。
 *
 * **D-6 のハックは 2026-08-08 に撤去した（1.3 → 1.0）。**
 * 以前の絵は横長で、48px枠に収めると高さが他の動物の6割ほどになり、
 * 「ひとまわり大きい」という設定が見た目に出なかったため 1.3倍にしていた。
 * 正方寄りに描き直して不要になった。実測（透過を除いた外接矩形 / 48px枠）:
 *
 *   critter_boar_s     44x48  高さ比 1.00  横長度 0.92
 *   critter_cat_s      47x48  高さ比 1.00  横長度 0.98
 *   critter_rabbit_s   27x48  高さ比 1.00  横長度 0.56
 *
 * 幅がうさぎの1.6倍あるので「ひとまわり大きい」は絵そのもので出ている。
 * `SPECIES_SCALE` の仕組みは残してある（別の種で必要になったときのため）。
 */
export const BOAR_SCALE = 1.0;
