...
import { loadSave, newCampaignSave, writeSave, type CampaignSave } from '../../src/app/save';
...
describe('T4-⑮ 連れ帰った者は累積で保存される', () => {
  it('新規セーブでは空配列から始まる', () => {
    expect(newCampaignSave().rescuedNames).toEqual([]);
  });
  ...
  it('旧セーブ（項目が無い JSON）でも空配列になり、読み込みが壊れない', () => {
    localStorage.clear();
    const save = newCampaignSave() as CampaignSave & { rescuedNames?: unknown };
