...
export const VEIL_CH10: MissionDef = {
  ...
  objectives: [
    ...
    // 守れなくても旗艦は落とせるので required にしない（「次に通る者のために撃つ」の任意目標）。
    {
      ...
      required: false,
      // 避難船（中立船）の生存／喪失がそのまま航路信頼に効く（全生存 +3 / 民間損害 -4/隻）
      reward: '＋航路信頼',
   ...
   * 読み上げるのは**自分が連れ帰った者の名前**だけ（`returneeRollCall()`）。
   * 第1章で収容した民間人も、灰冠回廊で拾った帝国籍の二名も、同じ一覧に並ぶ
   * ——「艦と勢力を問わず」。一人も連れ帰っていなければ、読む名前は無い。
   *
   * 累積分の出所は `Loadout.rescuedNames`（保存データ）で、
   * この出撃の分は `MissionRunner.summary().rescuedNames`。名簿をここに書かない。
   */
  rollCall: {
    navIndex: 2,
    speaker: CONFED_06,
    tone: 'command',
    intro: '最終無線を開く。勝者の名は読まない。帰ってきた者の名前だけを読む。',
    empty: '読み上げる名前が、今日は無い。門は開いたが、格納庫に降ろすものは何もなかった。',
    interval: 1.8,
  },
  debriefWin: [
