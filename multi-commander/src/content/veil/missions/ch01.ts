...
export const VEIL_CH01: MissionDef = {
  ...
  objectives: [
    ...
    //
    // ■「時間超過で輸送船が実際に沈む」挙動を入れない判断（T4-⑰ で再検討・結論は据え置き）
    // この目標は `required: true` の `timeLimit` なので、**300秒を超えたフレームで
    // `MissionRunner.evaluateObjectives()` が `finish('loss')` を呼び、その場で任務が終わる**。
    // つまり「沈む輸送船」を描く時間が原理的に存在しない（沈める処理を足しても誰も見られない）。
    // 見せられる形にするには隔壁のタイマーを必須から降ろすしかないが、
    // 「5分の制限時間も必須にする」は 06_更なる改善 で確定済みの方針なので変えない。
    // さらに沈めると `escortArrive`（到達不能）・`protect`（喪失）・`timeLimit`（超過）の
    // 3本が同フレームで失敗し、失敗理由が三重に出る。
    // 以上より**入れない**。時間超過が任務失敗になることは既にプレイヤーに伝わっている。
    {
      id: 'bulkhead',
