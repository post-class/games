...
export const VEIL_CH06: MissionDef = {
  ...
  objectives: [
    ...
    /**
     * 必須。中枢の応答を**記録として持ち帰る**（T3-C）。
     *
     * ■ なぜこの目標を required にしたか
     * 以前は必須の達成目標が `reachNav` 2本だけで、中枢の前を素通りして帰れば
     * 「任務達成」になっていた。この章で持ち帰らなければならないのは撃墜数ではなく
     * **原初核の工事報告そのもの**で、朝比奈の「反対意見も記録として残します」も、
     * 第7章の告発（同期ログが四つ目の証拠になる）も、記録が残っていることを前提にしている。
     *
     * ■ なぜ `destroyTag` にしなかったか
     * 中継器（`TAG.target`）の破壊を必須にすると「体温計を割っても熱は下がりません」という
     * 現場判断が選べなくなり、章末の CONSENT の二択（限定接続／完全遮断）が崩れる。
     * 護衛ドローン（`TAG.guard`）も、個体を失うことを死とみなさない相手なので
     * 全滅を勝利条件にできない。`rescue` / `holdTag` は
     * 対象（回収対象・維持対象）がこの戦場に存在しない。
     * 残るのは `recon`＝**撃たずに記録する**行為だけで、これは
     * 「中継器を1基も壊さずに中枢へ到達できる経路が存在する」前提を一切壊さない。
     *
     * 対象は統治空母（`TAG.capital`）。範囲を 2600m / 26° と広く取っているのは、
     * 相手が全長のある艦で、こちらは妨害圏の中でドローンに張り付かれながら
     * 機首を向け続けるためである（撃つ必要はない）。
     */
    {
      id: 'intent',
      text: '中枢の応答を記録する（原初核の工事報告を記録として持ち帰る。撃つ必要はない）',
      required: true,
      spec: { kind: 'recon', tag: TAG.capital, seconds: 10, range: 2600, coneDeg: 26 },
    },
    /**
     ...
     */
    {
      ...
      required: false,
      // 命令の履行そのもの。未達のまま帰れば「未達成の条件」として軍令信用が下がる
      reward: '＋軍令信用',
