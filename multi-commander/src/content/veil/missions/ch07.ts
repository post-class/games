...
export const VEIL_CH07: MissionDef = {
  ...
  spawns: [
    {
      // 公証中継所そのもの（T3-C）。書式の提出先を実体として置く。
      //
      // 五勢力の共同設備なので `faction: 'neutral'`（誰とも交戦しない）。
      // 船体に通信中継艦 `nn04-sky` を使うのは第8章と同じ流儀で、
      // 同じ戦域（公証中継所）に置いた設備が別の形に見えないようにするため。
      // `atNav: 2` なので中継所へ着いた瞬間に現れ、`role: 'transport'` なので撃ってこない。
      shipId: 'nn04-sky',
      count: 1,
      faction: 'neutral',
      atNav: 2,
      ...
      speed: 0,
      radio: [
        { speaker: '公証中継所', text: '書式の照合を開始する。回線を保て。', tone: 'command' },
      ],
    },
    {
  ...
  objectives: [
    ...
    },
    {
       ...
       *
       * ■ なぜ必要か
       * 以前は必須の達成目標が `reachNav`（中継所へ着く）だけで、
       * 中継所の座標を踏んだ瞬間に「任務達成」になっていた。だがこの章の条件は
       * ニアが読み上げたとおり「正規の書式で、期限内に、五勢力が検証できる形で出すこと」で、
       * 到着は前提でしかない。中継所へデータリンクを向けて保ち、照合を通す
       * ところまでを達成条件にする（追ってくる急進派を振り切ってからでないと保てない）。
       *
       * ■ なぜ `escortArrive` にしなかったか
       * この章の `TAG.escort` は**停止命令を執行する連邦の哨戒機**で、
       * 護送する対象ではなく進路を塞ぐ側である。彼らの到達を必須にすると
       *   (a) 「停止命令の執行者を中継所まで送り届ける」という筋の通らない目標になり、
       *   (b) 撃ってくる急進派（kilrathi）が哨戒機（confed）を落とした時点で任務失敗になる。
       *       この章は発砲そのものが代償になる設計なので、こちらは彼らを守れない
       *       ＝プレイヤーの操作では回避できない詰みが生まれる。
       * そこで、提出という**自分の操作だけで完結する行為**を必須にした。
       * 対象は中立の設備で誰にも撃たれないため、詰みにならない。
       */
      id: 'file',
      text: '中継所へ書式を提出する（回線を向けたまま保ち、五勢力が検証できる形で照合させる）',
      required: true,
      spec: { kind: 'recon', tag: TAG.survey, seconds: 12, range: 2000, coneDeg: 24 },
    },
    {
