...
/**
 * 1 試合の上限 tick。15,000 tick = 600 秒 = 10 分。
 *
 * 30 分（45,000 tick）フルは 1 試合 2.3 秒 × 168 試合 = 6 分半で予算に入らない。
 * `07§2` の区間では 10 分時点で「初接触（5〜12 分）」を抜けて分割戦に入っており、
 * 文明固有の兵種と固有令が一度は盤に出ている。**兵種ツリーの差が出る前に切る
 * わけではない**ので、バランスの指標としては足りると判断した。
 */
const MATCH_TICKS = 15000;

/** シード（マップ生成が変わる）。数値は任意だが**固定**する（回すたびに変わってはいけない）。 */
const SEEDS = [20260810, 777, 31337] as const;
