...
describe('文明バランス — 8 文明総当たり（T-M18-04）', () => {
   ...
   *
   * 勝率は 8 文明とも 50.0% に収まっているが、これは**均衡ではない**。
   * 同じ組・同じシードで左右を入れ替えると、**どちらの文明を置いても同じ席が勝つ**
   * （実測: yamato vs mongol → 席 1、mongol vs yamato → 席 1）。
   * つまり結果を決めているのは**文明ではなく開始位置**で、文明の違いは
   * 勝敗に 1 も効いていない。
   *
   * 原因は AI の内政の速さ。兵種ツリーは青銅の世で枝分かれするが、
   * AI が青銅に上がるのは 18〜24 分で、鉄器・帝国には 30 分でも届かない。
   * だから固有ユニット・エリート・火器・固有研究は**一度も盤に出ていない**。
   *
...
describe('この測定が有効であることの前提', () => {
  // 上のコメントの理由で現在は skip。AI の内政が速くなったら外す。
