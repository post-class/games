...
describe('文明バランス — 8 文明総当たり（T-M18-04）', () => {
   ...
   *
   * 文明差は盤に出るようになった（下の「この測定が有効であることの前提」が緑）。
   * ただし **まだ測れていないものがある**:
   *  - **試合が時間切ればかり**（`gameOver` 決着 0 件 / 112 試合）。
   *    勝敗は拠点数・人口・資源で決まっているので、**内政の測定に偏っている**。
   *  - AI が届くのは鉄器の世まで。**帝国の世（最終段の兵・火器）はまだ盤に出ていない**。
   *  - エリートは城が要るので、出番はさらに遅い。
   * つまりこの表は「30 分の内政 + 中盤の戦闘」の均衡であって、
   * **試合の終盤の均衡はまだ測っていない**。
   *
  ...
  //
  // ■ 実測の履歴（**新しい順**。数字を動かす前に必ず読むこと）
  //
  // 2026-08-11（効果適用エンジンを結線したあと。112 試合 / 2 シード）:
  //   ヴァイキング 64.3% / アステカ 57.1% / ローマ 53.6% / ペルシア 53.6% /
  //   マリ 46.4% / 唐 42.9% / モンゴル 42.9% / ヤマト 39.3%
  //   → **判定幅（42〜58%）を外れるのは 2 文明だけ**。外れているのは
  //     ヴァイキング(+14.3) と ヤマト(-10.7)。
  //
  // 2026-08-10（結線前。同じ 112 試合）:
  //   ローマ 78.6% / ペルシア 67.9% / アステカ 39.3% / 残る 5 文明 42.9%
  //
  // ■ この差が何を意味するか
  // 結線前の表は **文明の強みが半分死んだ状態**で測ったものだった
  // （研究 20 本・建物 3 件が効いていなかった。`docs/ISSUES.md` の重大項目）。
  // ローマ 78.6% の原因として「3 系統 3 段の副作用」を疑っていたが、
  // 結線したら **53.6% に収まった** ―― ローマの勝ち筋である街道（`03§4`
  // 「街道で戦域間の増援が最速」）が効いていなかったぶん、
  // **他文明の強みも効いていなかった**というのが実態だった。
  //
  // ■ 残る 2 文明について（次に見るところ）
  //  - ヴァイキング 64.3%: 内政ボーナスが「森 +15%」。木材は序盤いちばん細い資源なので
  //    同じ +15% でも効きが大きい。ローマに 100%、唐・マリ・モンゴルに 75% 勝つ。
  //  - ヤマト 39.3%: 内政ボーナスが「農地の食料 +15%」で、**農地が建つのは遅い**ので
  //    序盤に効かない。固有研究「鍛錬」も鉄器の世なので、いまの AI では出番が来ない。
  // どちらも「ボーナスの大きさ」ではなく **効き始める時刻**の差に見える。
  // 数値を動かす前にそこを確かめること。
