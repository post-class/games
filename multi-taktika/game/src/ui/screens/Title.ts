 ...
 *
 * ■ 未実装画面の扱い
 *   キャンペーン（M16）・リプレイ（M15）・オンライン（M14）はまだ無い。
...
/**
 * オンラインは画面（対戦設定）はあるが**通信が M14 で未実装**なので、
 * 画面の登録有無だけでは判定できない。ここだけ明示的に止めている。
 * M14 完了時にこの定数を false にすれば押せるようになる。
 */
export const ONLINE_PENDING = true;
